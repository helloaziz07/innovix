...
export default function AcceptInvite() {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      // Redirect to the shared project
      navigate(`/shared-projects/${details?.project_id}`)
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50">
      <motion.div 
        ...
        className="w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden p-8 text-center"
      >
        {error ? (
          <div className="flex flex-col items-center">
            ...
            <button 
              ...
              className="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-900 rounded-xl transition-colors font-medium"
            >
              Go to Dashboard
            </button>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-slate-500 mb-6">
              ...
              <span className="font-semibold text-slate-900">"{details.project_title}"</span>
            </p>
            
            <div className="w-full p-4 bg-slate-50 rounded-xl border border-slate-100 mb-8 flex items-center justify-between text-left">
              <div>
                <p className="text-xs text-slate-500 font-medium">Role</p>
                <p className="text-sm font-semibold text-slate-900 capitalize flex items-center gap-1">
                  <ShieldCheck className="w-4 h-4 text-green-500" />
                  {details.role}
                </p>
              </div>
              <div>
                <p className="text-xs text-slate-500 font-medium text-right">Invited Email</p>
            ...
            {!user ? (
              <div className="w-full space-y-3">
                <p className="text-sm text-slate-500 mb-2">You need to log in to accept.</p>
                <button 
